...
int process_data(CURL *curl_handle, RECV_BUF *p_recv_buf, char *curr_url)
{
    ...
    if (hinsert(eurl) == 1) // add effective url to log file if doesn't exist since top level url is already in from process http
    {
        if (log_file == 1)
...
static void init(CURLM *cm, char *url)
{
    ...
    curl_args *args = malloc(sizeof(curl_args));
    args->buf = recv_buf;
    args->url = url;

    curl_easy_setopt(eh, CURLOPT_PRIVATE, (void *)args);
...
int main(int argc, char **argv)
{
    ...
    while (img_count < m && url_frontier->head != NULL)
    {
        int i = 0;
        while (i < t && url_frontier->head != NULL)
        { // curl init for urls based on thread count
            node *temp = url_frontier->head;
            char *curr_url = malloc(strlen(temp->url) + 1);
            memcpy(curr_url, temp->url, strlen(temp->url) + 1);
            init(cm, curr_url);              // init
            url_frontier->head = temp->next; // move head of list to next
            i++;
            free(temp->url);
            free(temp);
        }
        ...
        while ((msg = curl_multi_info_read(cm, &msgs_left)))
        {
            if (msg->msg == CURLMSG_DONE)
            {
                ...
                if (return_code != CURLE_OK) // if CURL err, get info and cleanup malloc'd buffers and url
                {
                    // fprintf(stderr, "CURL error code: %d\n", msg->data.result);
                    curl_easy_getinfo(eh, CURLINFO_PRIVATE, (void *)&arguments);
                    curl_multi_remove_handle(cm, eh);
                    cleanup(eh, arguments->buf);
                    free(arguments->buf);
                    free(arguments->url);
                    free(arguments);
                    continue;
                ...
                process_data(eh, arguments->buf, arguments->url);
